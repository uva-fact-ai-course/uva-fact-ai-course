...
def AMAZON_dataset(args=None) :
    dataset = Dataset(name='amazon', path='preprocess/AMAZON/vec_amazon.p', args=args)
    set_balanced_pos_weight(dataset)
    return dataset   

def SMS_dataset(args=None) :
    dataset = Dataset(name='sms', path='preprocess/SMS/vec_sms.p', args=args)
    set_balanced_pos_weight(dataset)
    return dataset

def airline_dataset(args=None) :
    dataset = Dataset(name='airline', path='preprocess/USAirline/vec_airline.p', args=args)
    set_balanced_pos_weight(dataset)
    return dataset

def rotten_dataset(args=None) :
    dataset = Dataset(name='rotten', path='preprocess/ROTTEN/vec_rotten.p', args=args)
    set_balanced_pos_weight(dataset)
    return dataset    
