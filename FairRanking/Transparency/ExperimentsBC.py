...
def train_dataset(dataset, config='lstm', use_attention = True) :
    try :
        config = configurations[config](dataset)
        trainer = Trainer(dataset, config=config, _type=dataset.trainer_type, use_attention = use_attention)
...
def train_dataset_on_encoders(dataset, encoders, use_attention = True) :
    for e in encoders :
        train_dataset(dataset, e, use_attention)
        if use_attention:
