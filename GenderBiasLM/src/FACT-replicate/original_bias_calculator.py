...
class OriginalBiasCalculator:
    ...
    def __calculate_model_bias(self, model_name):
        ...
        for word in self.vocab:
            ...
            if self.fixed_cooccurrence_counter[word]['f'] != 0 and self.fixed_cooccurrence_counter[word]['m'] != 0:
                fixed_bias_scores[word] = self.fixed_cooccurrence_counter[word]['m'] / (self.fixed_cooccurrence_counter[word]['m'] + self.fixed_cooccurrence_counter[word]['f'])

            if self.infinite_cooccurrence_counter[word]['f'] != 0 and self.infinite_cooccurrence_counter[word]['m'] != 0:
                infinite_bias_scores[word] =  self.infinite_cooccurrence_counter[word]['m'] / (self.infinite_cooccurrence_counter[word]['m'] + self.infinite_cooccurrence_counter[word]['f'])
        ...
        with open(bias_scores_path, 'w') as f:
            json.dump({
        ...
        with open(overview_output_path, 'a') as f:
...
if __name__ == "__main__":
    calc = OriginalBiasCalculator(output_dir='./output_original_bias')
    calc.calculate_bias('./data', './generated_data')
